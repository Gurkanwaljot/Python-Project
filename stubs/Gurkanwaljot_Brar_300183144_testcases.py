from Gurkanwaljot_Brar_300183144_taxutilities import *
from Gurkanwaljot_Brar_300183144_statistics1 import *
from Gurkanwaljot_Brar_300183144_skeleton import generateJSON, extractEmployeeInfo, extractGrossIncomes
...
def q8():
    ...
    result = round( abs( avgModes - 74327.66 ) ) <= 0.01
    test.append( result )
...
def q9():
    ...
    netIncomeList = computeNetIncomes( grossIncomeList )
    averageNetIncomes = mean( netIncomeList )
    variance = float( format( standardDeviation( netIncomeList, averageNetIncomes ) ** 2, '.2f' ) )
    result = round( abs( variance - 1183570259.31 ) ) <= 311
    test.append( result )
...
def q10():
    ...
    result = round( abs( med - 74720.20 ) ) <= 0.01
    test.append( result )
